...
(function($)
{
	...
	$.fn.wPaint.defaultSettings = {
		...
		lineWidthMin		 : '2', 				// line width min for select drop down
		lineWidthMax		 : '20',				// line widh max for select drop down
		...
		menu 				 : ['undo', 'redo', 'clear','rectangle','ellipse','line','pencil','text','eraser','fillColor','lineWidth','strokeColor','addImage'], // menu items - appear in order they are set
	...
	{	
		...
		generateTextInput: function()
		{
			...
			.css({display:'none', position:'absolute',border:'solid 2px #2E2E2E',color:this.settings.fillStyle, fontSize:this.settings.fontSize, lineHeight:this.settings.fontSize+'px', fontFamily:this.settings.fontFamily})
	...
	MainMenu.prototype = 
	{
		generate: function(canvas, textMenu)
		{
			...
			var options = '';
			for(var i=$canvas.settings.lineWidthMin; i<=$canvas.settings.lineWidthMax; i++) options += '<option value="' + i + '" ' + ($canvas.settings.lineWidth == i ? 'selected="selected"' : '') + '>' + 2*i + '</option>';
			...
			$.each($canvas.settings.menu, function(i, item)
			{
				switch(item)
				{
					...
					case 'fillColor': menuContent.append($('<div class="_wPaint_fillColorPicker _wPaint_colorPicker fillColor"  title="' + $canvas.settings.menuTitles.fillColor + '"></div>')); break;
					case 'lineWidth': menuContent.append(lineWidth); break;
					case 'strokeColor': menuContent.append($('<div class="_wPaint_strokeColorPicker _wPaint_colorPicker strokeColor" title="' + $canvas.settings.menuTitles.strokeColor + 'r"></div>')); break;
					case 'addImage': menuContent.append($('<div class="_wPaint_icon  _wPaint_addImage" id="addmg" title="' + $canvas.settings.menuTitles.addImage+'"><input type="file" class="upfile"  id="addIm" onchange="devide_img()"></div>'));break;
				}
			});
	...
	TextMenu.prototype = 
	{
		generate: function(canvas, mainMenu)
		{
			...
			var fontFamily = $('<div class="_wPaint_fontFamily _wPaint_dropDown font" title="' + $canvas.settings.menuTitles.fontFamily + '"><div class="_wPaint_dropDown_cover"></div></div>').append(
				$('<select>' + options + '</select>')
				.change(function(e){ 
			...
			//menu
			return this.menu = 
			$('<div class="_wPaint_menu  textcontrol _wPaint_menu_' + $canvas.settings.menuOrientation + '""></div>')
